            ...
            <form id="buy-form" action="{{ url('create-order') }}" method="post">
                ...
                <div class="form-group">
                    ...
                    @if($buy_limit_num > 0)
                        <span class="badge badge-outline-dark">
                            {{__('hyper.buy_purchase_restrictions')}}({{ $buy_limit_num }})</span>
                    @endif
                </div>
                    ...
                    <div class="form-group">
                        <div class="alert alert-dark bg-white text-dark mb-0" role="alert">
                            ...
                            @foreach($wholesale_price_cnf as $ws)
                                {{-- 购买 x 件起， x 元/件 --}}
                                <span>
                                    {{ __('hyper.buy_purchase') }} {{ $ws['number'] }}
                                    {{__('hyper.buy_the_above')}}，{{ $ws['price']  }} {{__('hyper.buy_each')}}。
                                </span>
                            @endforeach
                        </div>
                    </div>
                ...
                @if(dujiaoka_config_get('is_open_search_pwd') == \App\Models\Goods::STATUS_OPEN)
                    <div class="form-group">
                        ...
                        {{-- 查询订单密码 --}}
                        <input type="text" name="search_pwd" value="" class="form-control"
                    ...
                    @foreach($other_ipu as $ipu)
                        <div class="form-group">
                            ...
                            <input type="text" name="{{ $ipu['field'] }}" @if($ipu['rule'] !== false) required @endif
                                class="form-control" placeholder="{{ $ipu['desc'] }}">
                        </div>
                    @endforeach
                @endif
                ...
                @if(dujiaoka_config_get('is_open_img_code') == \App\Models\Goods::STATUS_OPEN)
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="text" name="img_verify_code" value="" class="form-control"

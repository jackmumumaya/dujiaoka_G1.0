      ...
      <div class="card card-body sticky">
        <form id="buy-form" action="{{ url('create-order') }}" method="post">
          ...
          <div class="form-group">
            ...
            <input type="email" name="email" class="form-control" placeholder="接收卡密或通知" required>
            @error('email')
          ...
          @if($type == \App\Models\Goods::MANUAL_PROCESSING && is_array($other_ipu))
            @foreach($other_ipu as $ipu)
              <div class="form-group">
                ...
                @endif
                <input type="text" class="form-control" id="{{ $ipu['field'] }}" name="{{ $ipu['field'] }}" @if(
                  $ipu['rule']
                  !== false
                ) required @endif placeholder="{{ $ipu['desc'] }}">
              </div>
            @endforeach
          @endif
                  ...
                  <div class="btn pay-type   @if($index == 0) active @endif  " data-type="{{ $way['pay_check'] }}"
                    ...
                    @if(!empty($way['icon_path']))
                      <img src="{{ picture_ulr($way['icon_path']) }}" alt="{{ $way['pay_name'] }}"
                        style="width: 24px; height: 24px; margin-right: 5px; vertical-align: middle;">
                    @endif
                    {{ $way['pay_name'] }}
                  </div>
                @endforeach

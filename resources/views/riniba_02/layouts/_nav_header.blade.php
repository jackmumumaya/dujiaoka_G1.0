        ...
        <nav class="header-inner flx-between">
            ...
            <div class="header-right flx-align">
                ...
                <div class="header-right__inner gap-3 flx-align d-lg-flex d-none">
                    ...
                    @guest
                        <a href="{{ route('login') }}" class="btn btn-main pill"
                            style="background: transparent; border: 1px solid currentColor;">登录</a>
                        <a href="{{ route('register') }}" class="btn btn-main pill">注册</a>
                    @else
                        <span style="color: var(--white); font-weight: 500;">{{ Auth::user()->name }}</span>
                        <form method="POST" action="{{ route('logout') }}" style="display: inline;">
                            @csrf
                            <button type="submit" class="btn btn-main pill"
                                style="background: transparent; border: 1px solid currentColor;">退出</button>
                        </form>
                    @endguest

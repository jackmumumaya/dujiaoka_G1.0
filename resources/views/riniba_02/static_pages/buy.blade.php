                ...
                <div class="d-flex flex-wrap gap-2">
                  @foreach($payways as $key => $way)
                    <div class="form-check me-3">
                      <input class="form-check-input" type="radio" name="payway" id="payway_{{$way['id']}}"
                        value="{{$way['id']}}" {{$key == 0 ? 'checked' : ''}}>
                      <label class="form-check-label d-flex align-items-center" for="payway_{{$way['id']}}">
                        @if(!empty($way['icon_path']))
                          <img src="{{ picture_ulr($way['icon_path']) }}" style="height: 20px; margin-right: 5px;">
                        @endif
                        {{ $way['pay_name'] }}
                      </label>
                    </div>
